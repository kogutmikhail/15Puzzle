...
TileIndex::TileIndex(int x_, int y_){
	y = y_;
	x = x_;
}
...
void Game::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (unsigned int x = 0; x < SIZE; x++)
	{
		...
		{	
			...
			text.setPosition(position.x + 40.f + (grid[x][y] < 10 ? 30.f : 0.f), position.y + 25.f);
